...
export default function RegistrarDashboard() {
  const { data, isLoading } = useQuery({
    queryKey: ['applications', 'all', 'registrar'],
    queryFn: () => applicationsApi.listAll({ status: 'REGISTRAR_REVIEW' }),
  });
  ...
  return (
    <div className="flex flex-col h-full">
      <Header title="Registrar Dashboard" />
      <div className="flex-1 overflow-auto p-6 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <h2 className="text-base font-semibold">Pending Final Approval</h2>
          <Button asChild variant="outline" size="sm">
            <Link href="/registrar/queue">View All</Link>
          </Button>
        </div>
          ...
          <CardContent className="p-0">
            {isLoading ? (
              ...
              <div className="p-12 text-center">
                ...
                <p className="text-sm font-medium">No applications awaiting approval</p>
              </div>
            ) : (
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y">
                  {queue.slice(0, 10).map((app: { id: string; trackingNumber: string; student?: { email: string }; attestationType: string; status: string; createdAt: string }) => (
